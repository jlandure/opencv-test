...
class height_resource : public http_resource {
	public:
...
void height_resource::render(const http_request& req, http_response** res)
{
    ...
    Mat m = imread("Zenika-Logo-300x140.png");
    int height=getHeight(m);
	ostringstream response;
	response << "Height of logo:" << height << "px.";
    ...
    *res = new http_response(http_response_builder(response.str(), 200).string_response());
}
...
int main()
{
    ...
    //method is called.
    ws.register_resource("/height", &hwr, true);
